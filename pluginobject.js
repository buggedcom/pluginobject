...
// 		automagically guess plugin type from extension
	...
	loadPlugin: function(p, c,  o)
	{
		...
		{
			document.write('<scr'+'ipt type="text/javascript" src="'+l+'" onreadystatechange="PO.U._loadPlugin(\''+p.src+'\', '+i+');" onload="PO.U._loadPlugin(\''+p.src+'\', '+i+');"></'+'script>');
		}
	},
	_lpref: {},
	_loadPlugin: function(ref, i, e)
	{
		if(event.type == 'readystatechange' && event.srcElement.readyState != 'complete') return;
